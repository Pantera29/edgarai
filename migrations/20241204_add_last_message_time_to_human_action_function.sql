-- MIGRACIÓN: Agregar campo last_message_time a la función de acción humana
-- Fecha: 2024-12-04
-- Objetivo: Incluir el timestamp del último mensaje para mostrar la fecha/hora correcta

-- Eliminar la función existente para recrearla con el nuevo campo
...
RETURNS TABLE (
  ...
  -- Campo para el último mensaje (nuevo)
...
BEGIN
  ...
  SELECT 
    ...
    cc.created_at,
    cc.updated_at,
    CAST(cc.duration_seconds AS INTEGER),
    cc.ended_reason,
    cc.was_successful,
    ...
    cc.messages,
    cc.last_read_at,
    -- Campo para el último mensaje (extraer del JSONB messages)
    (
      SELECT MAX(
        CASE 
          WHEN jsonb_typeof(msg->'created_at') = 'string' THEN (msg->>'created_at')::TIMESTAMPTZ
          WHEN jsonb_typeof(msg->'timestamp') = 'string' THEN (msg->>'timestamp')::TIMESTAMPTZ
          WHEN jsonb_typeof(msg->'time') = 'string' THEN (msg->>'time')::TIMESTAMPTZ
          ELSE NULL
        END
      )
  ...
  WHERE cc.dealership_id = p_dealership_id
    ...
    AND (p_urgency_filter = 'all' OR 
         EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - cc.updated_at)) / 3600 <= v_urgency_threshold_hours)
  ORDER BY 
    -- Ordenar por último mensaje primero (más recientes primero)
    COALESCE(
      (
        SELECT MAX(
          CASE 
...
COMMENT ON FUNCTION get_conversations_needing_human_action IS 'Obtiene conversaciones que necesitan acción humana (clientes con agent_active = false) con timestamp del último mensaje';
