...
export default function NuevoAsesorPage() {
  ...
  const [formData, setFormData] = useState<CreateServiceAdvisorInput>({
    dealership_id: "",
    workshop_id: "",
    name: "",
    email: "",
    ...
    if (searchParams) {
      ...
      if (tokenValue) {
        ...
        if (verifiedDataToken?.dealership_id) {
          cargarWorkshops(verifiedDataToken.dealership_id)
        }
      }
    }
  }, [searchParams, router])
  ...
  async function cargarWorkshops(dealershipId: string) {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "No se pudieron cargar los talleres",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  function validateForm(): boolean {
    ...
    if (formData.lunch_start_time < formData.shift_start_time || 
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch('/api/service-advisors', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || data.error || 'Error al crear asesor')
      }

      toast({
        title: "Asesor creado",
        description: data.message || "El asesor ha sido creado exitosamente",
      })
  ...
  function handleCheckboxChange(day: keyof CreateServiceAdvisorInput, checked: boolean) {
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
            ...
            Nuevo Asesor de Servicio
          </h1>
          <p className="text-muted-foreground mt-1">
            Completa la información del nuevo asesor
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={day.key} className="flex items-center space-x-2">
                  <Checkbox
                    id={day.key}
                    checked={formData[day.key as keyof CreateServiceAdvisorInput] as boolean}
                    onCheckedChange={(checked) => 
                      handleCheckboxChange(day.key as keyof CreateServiceAdvisorInput, checked as boolean)
                    }
                  />
